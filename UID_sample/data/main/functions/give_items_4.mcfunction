...
# 使用するUIDをセット
execute as @s at @s run data modify storage test UID_setter set from storage test UID[3]

# UIDを付与する
execute as @s at @s run item modify entity @e[tag=item_generator] weapon.mainhand main:uid_set

# 受け取り用アイテムを召喚
execute as @s at @s run summon item ~ ~ ~ {PickupDelay:1,Item:{id:"minecraft:leather_leggings",Count:1b}}

# データのコピー
execute as @s at @s run data modify entity @e[type=item,nbt={Item:{id:"minecraft:leather_leggings"}},sort=nearest,limit=1] Item.tag set from entity @e[tag=item_generator,limit=1] HandItems[].tag
