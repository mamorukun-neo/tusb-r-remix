...
# 受け取り用アイテムを召喚
summon item ~ ~ ~ {PickupDelay:1,Item:{id:"minecraft:elytra",Count:1b},Tags:[konton]}

# データのコピー
data modify entity @e[type=item,nbt={Item:{id:"minecraft:elytra"}},sort=nearest,limit=1,tag=konton] Item.tag set from entity @e[tag=item_generator,limit=1] HandItems[].tag
